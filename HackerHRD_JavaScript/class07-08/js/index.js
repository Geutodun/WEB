...
createBtn.addEventListener("click", () => {
    // 생성하기 버튼을 클릭했을 경우, 생성페이지로 이동
    location.href = "create.html"
})

// ------------------------------------------------------------------------

// 로컬 스토리지에 있는 데이터를 기준으로 LIST UI 추가
const posts = JSON.parse(localStorage.getItem("posts"))
const postIds = []

if(posts.length === 0) {
    bodyEl.innerHTML = `
        <div class="container_body_noData">
    ...
    posts.forEach((post) => {
        bodyEl.innerHTML += `
            <div class="container_body_list">
                ...
                <span class="title">작성자 : ${post.user}</span>
                <spam class="content">내용 : ${post.content}</spam>
