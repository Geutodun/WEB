...
// Javascript는 웹 페이지에서 복잡한 기능을 구현할 수 있는 스크립팅 혹은 프로그래밍 언어 입니다.
// 자바스크립트로 작성한 프로그램을 스크립트(script)라고 부릅니다.
// 스크립트는 웹 페이지의 HTML 안에 작성할 수 있는데, 웹 페이지를 불러올 때 스크립트가 자동으로 실행됩니다.

// 사용자는 컴퓨터에서 웹에 접근 => 서버에 최초로 요청 (Request) => 최조로 응답(Response)
// 최초로 응답받은 데이터 혹은 정보 안에는 기본적인 구조를 담은 HTML이 응답으로 전달됩니다.
// => 추가 요청(CSS, HTML, JS, 이미지, 비디오 등) => 추가 응답
...
// 따라서 시본 타입에는 문자열, 숫자, 불(boolean), null, undegined가 있다.

// -------------------------------------------------------------------------------------------------------------------
// 1. 문자열
// 문자열 데이터는 만드는 방법은 큰 따옴표, 작은 따옴표, 백틱(``)으로 문자를 감싸면 됩니다.
...
console.log(typeof valueTrue, valueTrue)
console.log(typeof valueFalse, valueFalse)
...
// 여기서 null 타입은 object를 반환하는 것을 확인
// 이를 미루어볼 때, null은 '객체가 없다'라는 것을 의미 합니다.
let obj = { 
    userName : "HACKERS HRD",
    uesrPhone : "010-1234-5678"
}
...
//let undef
//consloe.log(typeof undef, undef)
...
// 즉, 객체는 키-벨류(key-value) 형태로 저장
let userInfo = {
    userName : "Kim",
    userAge : 30,
    major : "?",
    email : "Hackers@hackers.com",
    married : false
}
